...
export default () => {
  ...
  //最近阅读数据
  const [recentlyList] = useState<Partial<bookProps>[]>([
    {
      id: 1,
      title: '赶尸列车',
  ...
  //书架数据
  const [bookList] = useState<Partial<bookProps | null>[][]>([
    [
      {
        id: 3,
  ...
  //需要删除的id list
  const [delArray, setDel] = useState<number[]>([]);

  //edit为false时清空delArray
  useEffect(() => {
    if (!edit) setDel([]);
  }, [edit]);
  //改变多选框
        ...
        <div className={`bookShelf_face_mask ${edit ? 'showMask' : ''}`}>
          {edit ? (
            <Checkbox
              ...
              checked={delArray.includes(book.id as number)}
              onChange={(e) => onChangeCheckBox(e, book.id as number)}
            />
          ) : (
            <IconFont className={'bookShelf_face_icon'} icon={'delete'} />
          )}
          {children}
        </div>
      </div>
  ...
  // 最近阅读
  const RecentlyRead = ({ book }: { book: Partial<bookProps> }) => {
    return (
