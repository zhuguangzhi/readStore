...
export const AddSection = () => {
  ...
  // 作品详情
  const { data: worksInfo } = uesGetAuthorBookDetails({ id: Number(worksId) });
  // 主要用于默认值
  ...
  const saveWorks = async (draft: 1 | 2) => {
    ...
    const container = editorEl?.getContent() || '';
      ...
      <div className={'admin_container addSection_container'}>
        ...
        <div className={'addSection_container_box'}>
          ...
          <div style={{ flex: 1, height: '100%' }}>
            <UseNode rIf={!chapterDetailsLoading}>
              <ReadEditor
            ...
            {/*  驳回缘由*/}
            <UseNode rIf={chapterDetails?.reject_reason}>
              <div className={'addSection_container_reject'}>
                <span>{chapterDetails?.reject_reason}</span>
              </div>
            </UseNode>
          </div>
        </div>
